...
import { IPlanet, PlanetName, TypeButtons } from 'src/app/interfaces/globals';
...
export class PlanetComponent {
	...
	ngOnInit(): void {
		this.route.paramMap.subscribe(params => {
			let _planetName = (params.get('planet') || 'earth') as PlanetName;
			this.setActualPlanet(_planetName)
		});
	}

	protected selectContentType(contentType: TypeButtons): void {
		this.selectedContent = this.planet[contentType]
		this.selectedButton = contentType;
		this.setImg()
	}

	private setActualPlanet(planetName:PlanetName): void {
		this.selectedButton = 'overview';
		this.planet = this.planets.find((planet:IPlanet) => planet.name === planetName) as IPlanet;
		this.selectedContent = this.planet.overview;
		this.setImg();
	}

	private setImg(){
		if(this.selectedButton === 'overview'){
			this.imgSrc = this.planet.images.planet;
		}
		if(this.selectedButton === 'structure'){
			this.imgSrc = this.planet.images.internal;
		}
		if(this.selectedButton === 'geology'){
			this.imgSrc = this.planet.images.geology;
		}
	}
	...
	// options for button styles: overview | structure | geology
	protected selectedButton:TypeButtons = 'overview'; 
	// for selected link and description:
	protected selectedContent:{content:string, source:string} = this.planet.overview;
	// for img src:
	protected imgSrc:string = './assets/planet-earth.svg';


	protected planets: IPlanet[] = [
		{
		  name: "mercury",
		  overview: {
			content:
